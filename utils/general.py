...
def feature_visualization(x, module_type, stage, n=512, save_dir=Path('runs/detect/exp')):
    """
    ...
    if 'Detect' not in module_type:
        ...
        if height > 1 and width > 1:
            ...
            for i in range(n):
                f = save_dir /f"stage{stage}_{module_type.split('.')[-1]}_{i}_features.png"  # filename
                ...
                fig = plt.subplots(1, 1, tight_layout=True)  # 8 rows x n/8 cols
                # ax = ax.ravel()
                plt.subplots_adjust(wspace=0.05, hspace=0.05)

                plt.imshow(temp, cmap=plt.cm.jet)  # cmap='gray'
                plt.axis('off')
...
#     model_type: The type of feature map    # model_type通常指想要进行可视化的模块的名称
#     model_id: The id of feature map        # model_id通常指想要进行可视化的模块所在的层数  参考.yaml文件
...
#     # block by channel dimension 沿着通道进行分块         torch.chunk(tensor, chunks, dim) 在给定的维度上对张量进行分块
#                                                  #参数说明：tensor(Tensor)-待分块的输入张量  chunks(int)-分块的个数  dim(int)-维度，沿着此维度进行分块
...
#                      .format(model_type.split('.')[2], model_id, i), dpi=300)     # 在这里保存 特征图会一张一张的保存
#         # gray feature
#         # plt.imshow(feature, cmap='gray')
#
#     # plt.show()
#     # plt.savefig(save_dir + '{}_{}_feature_map_{}.png'
#     #             .format(model_type.split('.')[2], model_id, feature_num), dpi=300)     # 在这里保存  特征图会排列在一起保存为一张图
