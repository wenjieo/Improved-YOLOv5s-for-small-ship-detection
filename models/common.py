...
class Bottleneck(nn.Module):
    ...
    def __init__(self, c1, c2, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, shortcut, groups, expansion
        # shortcut：是否给bottleneck结构部添加shortcut连接，添加后即为ResNet模块；(是否进行残差连接)
        # Yolov5中的backbone中的Bottleneck都默认使shortcut为True，在head中的Bottleneck都不使用shortcut。
...
class SPP(nn.Module):
    ...
    def __init__(self, c1, c2, k=(5, 9, 13)):   # 这里的5、9、13是初始化的kernel size
...
class Add(nn.Module):
    ...
    def forward(self, x):
        return self.conv(self.silu(x[0] + x[1] + x[2]))
...
# class MHSA(nn.Module):
#     def __init__(self, n_dims, size, heads=4, pos_emb=False):
#         super(MHSA, self).__init__()
#
#         height = size
#         width = size
...
class MSFE(nn.Module):
    ...
    def __init__(self, c_in, c_out):
        ...
        self.branch3 = nn.Sequential(
            nn.Conv2d(c_in, c_, (3, 1), stride=1, padding=(1, 0)),
            nn.BatchNorm2d(c_),
            nn.SiLU(),
            nn.Conv2d(c_, c_, (1, 3), stride=1, padding=(0, 1)),
            nn.BatchNorm2d(c_),
            nn.SiLU()
        )
        ...
        self.branch4 = nn.Sequential(
            ...
            nn.BatchNorm2d(c_),
            nn.SiLU(),
            nn.Conv2d(c_, c_, (3, 1), stride=1, padding=(1, 0)),
            nn.BatchNorm2d(c_),
            nn.SiLU(),
            nn.Conv2d(c_, c_, (1, 3), stride=1, padding=(0, 1)),
            nn.BatchNorm2d(c_),
        ...
        self.branch5 = nn.Sequential(
            nn.Conv2d(c_in, c_, (5, 1), stride=1, padding=(2, 0)),
            nn.BatchNorm2d(c_),
            nn.SiLU(),
            nn.Conv2d(c_, c_, (1, 5), stride=1, padding=(0, 2)),
            nn.BatchNorm2d(c_),
            nn.SiLU(),
            nn.Conv2d(c_, c_, (5, 1), stride=1, padding=(2, 0)),
            nn.BatchNorm2d(c_),
            nn.SiLU(),
            nn.Conv2d(c_, c_, (1, 5), stride=1, padding=(0, 2)),
            nn.BatchNorm2d(c_),
        ...
        self.conv = nn.Conv2d(c_in*2, c_out, kernel_size=1)
